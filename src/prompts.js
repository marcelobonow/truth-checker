...
Neste servidor você só tem acesso às ferramentas de busca na web (WebSearch e WebFetch).` +
    ' Você não tem acesso a arquivos locais arbitrários nem a comandos. Porém, consegue ler arquivos enviados como anexo no Discord: arquivos de texto e código (.txt, .csv, .tsv, .md, .json, .jsonl, .log, .yaml/.yml, .xml, .ini, .toml, .properties, .js/.ts e similares), Word (.doc e .docx), PDF (.pdf) e OpenDocument Text (.odt).',
  full: (workDir) =>
    COMMON_PROMPT +
    `
